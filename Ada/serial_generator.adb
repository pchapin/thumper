...
-- instantiated in a unit with SPARK mode off. This limitation may be lifted in the future. I also tried putting the
-- instantiation in its own compilation unit but that didn't help. Any use of the facilities of the instantiated module were
-- flagged as not in SPARK.
...
package body Serial_Generator
  with Refined_State => (Number => Current_Number) is

   package Serial_Number_IO is new SPARK.Text_IO.Modular_IO(Serial_Generator.Serial_Number_Type);
   ...
   function Get return Serial_Number_Type
     with Refined_Global => (Input => Current_Number)
   is
   begin
